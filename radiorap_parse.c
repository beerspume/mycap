...
int parseRadioHeader(const u_char* pkg_data,struct std_rt_header* header){
	...
	read_p+=2;
	header->len=*(uint16_t*)read_p;
	read_p+=sizeof(uint16_t);
	header->present=*(uint32_t*)read_p;
	...
	while((search_present&(1<<RADIOTAP_PRESENC_EXT))){
		...
		search_present=*(uint32_t*)read_p;
		read_p+=sizeof(uint32_t);
	}

	if(header->present&(1<<RADIOTAP_PRESENC_TSFT)){
		header->has_TSFT=1;
		header->v_TSFT=*(uint64_t*)read_p;
	...
	if(header->present&(1<<RADIOTAP_PRESENC_CHANNEL)){
		...
		header->v_Channel_Frequency=*(uint16_t*)read_p;
		read_p+=sizeof(uint16_t);
		header->v_Channel_Flags=*(uint16_t*)read_p;
		read_p+=sizeof(uint16_t);
	}
	...
	if(header->present&(1<<RADIOTAP_PRESENC_RX_FLAGS)){
		...
		header->v_RXFlags=*(uint16_t*)read_p;
		read_p+=sizeof(uint16_t);
	}
	...
	if(header->present&(1<<RADIOTAP_PRESENC_AMPDU_STATUS)){
		...
		header->v_AMPDUStatus_RefNum=*(uint32_t*)read_p;
		read_p+=sizeof(uint32_t);
		header->v_AMPDUStatus_Flags=*(uint16_t*)read_p;
		read_p+=sizeof(uint16_t);
	...
	if(header->present&(1<<RADIOTAP_PRESENC_VHT)){
		header->has_VHT=1;
		header->v_VHT_Known=*(uint16_t*)read_p;
		read_p+=sizeof(uint16_t);
		...
		read_p+=sizeof(uint8_t);
		header->v_VHT_Partialaid=*(uint16_t*)read_p;
		read_p+=sizeof(uint16_t);
	}
